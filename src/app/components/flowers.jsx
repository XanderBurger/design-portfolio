...
export default function Sketch({ backgroundColor, frameRate }) {
  function drawing(p5) {
    const container = document.getElementById("flower-sketch-container");
    ...
    function setGridCell(x, y, colorNumber) {
      const NOT_INSIDE =
        x > width / 10 - 1 || x < 1 || y > height / 10 - 1 || y < 1;
    ...
    function rule(ruleNumber, cell, neighbors) {
      const upClear =
        grid[neighbors.up[0]][neighbors.up[1]][0] === 0 &&
      ...
      if (ruleNumber === 0) {
        ...
        if (grid[cell[0]][cell[1]][0] === 1 && upClear) {
          ...
          if (randNum === 1) {
            setGridCell(neighbors.up[0], neighbors.up[1], 1);
          }
          if (randNum === 2) {
      ...
      if (ruleNumber === 2) {
        if (grid[cell[0]][cell[1]][0] === 5) {
          if (upClear && flowerCount < 3) {
            if (randFlower === 0) {
              setGridCell(neighbors.upRight[0], neighbors.upRight[1], 5);
            }
            if (randNum === 1) {
    ...
    function drawCells() {
      ...
      const GREEN = p5.color("#1ee387");
      const YELLOW = p5.color("#f5e90c");
      const PINK = p5.color("#FF2CFA");
      const BLUE = p5.color("#21d6ff");
    ...
    p5.draw = () => {
      ...
      updateGrid(ruleNum);
      drawCells();
      p5.fill(p5.color("#1ee387"));
      p5.rect(
        Math.round(p5.mouseX / 10 - 1) * 10,
        Math.round(p5.mouseY / 10 - 1) * 10,
        10,
        10
      );
    };
    ...
    p5.mouseReleased = () => {
      setGridCell(
        Math.round(p5.mouseX / 10 - 1),
        Math.round(p5.mouseY / 10 - 1),
        1
      );
      ruleNum = (1 + ruleNum) % 3;
    };
